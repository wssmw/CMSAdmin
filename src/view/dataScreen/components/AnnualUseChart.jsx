...
const AnnualUseChart = memo(() => {
    ...
    const option = {
		tooltip: {
          ...
          <div className="year-item">
            <span className="year-dot" style={{backgroundColor: ${val.color}}}></span>
            <span className="year-name">${val.seriesName}</span>
            <span className="year-value">${val.data >= 10000 ? (val.data / 10000).toFixed(2) + "w" : val.data}</span>
          </div>
          `;
				});
				let dom = `
                    <div className="annual-tooTip">
                      <span className="annual-month">${p[0].dataIndex + 1}月</span>
    ...
    useEffect(() => {
        ...
        var myChart = echarts.init(document.querySelector(".AnnualUseChart_main"));
        // 绘制图表
  ...
  return (
    <div style={{width:"100%",height:"100%"}}>
       <div className="AnnualUseChart_main" style={{width:"100%",height:"100%"}}></div> 
    </div>
  )
})
